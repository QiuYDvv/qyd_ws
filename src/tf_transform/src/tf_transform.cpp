  ...
  void cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg)
  {
    ...
    else if (velocity_mode == "odom")
    {
      ...
      try
      {
        ...
        tf::Stamped<tf::Vector3> vel_world_stamped(vel_world, ros::Time(0), "odom");

        tf::Stamped<tf::Vector3> vel_base_stamped;  // 用于存储转换后的速度向量

        // 调用 transformVector
        tf_listener.transformVector("base_link", vel_world_stamped, vel_base_stamped);
        ...
        // 计算轮速并执行
        inverseKinematics(transformed_cmd_vel.linear.x, transformed_cmd_vel.linear.y, transformed_cmd_vel.angular.z, v1,
                          v2, v3, v4);
        excuteWheelSpeeds(v1, v2, v3, v4);
      }
...
private:
  ...
  ros::Publisher left_front_wheel_pub =
      nh.advertise<std_msgs::Float64>("/controller/left_front_wheel_controller/command", 50);
  ros::Publisher right_front_wheel_pub =
      nh.advertise<std_msgs::Float64>("/controller/right_front_wheel_controller/command", 50);
  ros::Publisher left_back_wheel_pub =
      nh.advertise<std_msgs::Float64>("/controller/left_back_wheel_controller/command", 50);
  ros::Publisher right_back_wheel_pub =
      nh.advertise<std_msgs::Float64>("/controller/right_back_wheel_controller/command", 50);
